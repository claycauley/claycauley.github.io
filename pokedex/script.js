            ...
            if (data.types) {
                ...
                if (detailHeader && types.length > 0) {
                    if (types.length === 2) {
                        ...
                        const headerGradient = `linear-gradient(135deg, ${type1Rgba} 45%, ${type2Rgba} 55%), #000000`;
                        detailHeader.style.background = headerGradient;
                    } else if (types.length === 1) {
                        const type1Color = typeColors[types[0]] || '#A8A878';
                        const type1Rgba = hexToRgba(type1Color, 0.15);
                        const headerGradient = `linear-gradient(90deg, ${type1Rgba} 0%, ${type1Rgba} 100%), #000000`;
                        detailHeader.style.background = headerGradient;
                    }
                }
            }
