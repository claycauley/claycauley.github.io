...
// =============================================
// Send email via SMTP (Hostinger disables mail())
// =============================================

// ⚠️ SMTP credentials — use your real Hostinger email account
define('SMTP_HOST', 'smtp.hostinger.com');
define('SMTP_PORT', 465);
define('SMTP_USER', 'contact@claydesigns.cc');       // <-- your Hostinger email
define('SMTP_PASS', 'Nuclei$Salvation$Pluck$Tipped$Railcar5');      // <-- that email's password
...
$rawMessage .= "To: " . MAIL_TO . "\r\n";
$rawMessage .= "Reply-To: {$replyTo}\r\n";
$rawMessage .= "Subject: " . MAIL_SUBJECT . " from {$name}\r\n";
...
try {
    ...
    $sendCmd = function($socket, $cmd = null) {
        if ($cmd !== null) {
            fwrite($socket, $cmd . "\r\n");
        }
        $response = '';
    ...
    $sendCmd($socket);                                                  // Read greeting
    $sendCmd($socket, 'EHLO ' . gethostname());                        // EHLO
    $sendCmd($socket, 'AUTH LOGIN');                                    // Start auth
    $sendCmd($socket, base64_encode(SMTP_USER));                        // Username
    $authResponse = $sendCmd($socket, base64_encode(SMTP_PASS));        // Password
    ...
    $sendCmd($socket, 'RCPT TO:<' . MAIL_TO . '>');
    $sendCmd($socket, 'DATA');
    $sendCmd($socket, $rawMessage . "\r\n.");
    $sendCmd($socket, 'QUIT');
